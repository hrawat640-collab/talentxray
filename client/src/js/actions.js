...
function handleOpenClick(e){const lnk=document.getElementById('openGoogleLink');if(!lnk||lnk.href===window.location.href||lnk.getAttribute('href')==='#'){toast('Add a job title or skill first','err');e.preventDefault();return false;}const ti=document.getElementById('titleInput');if(ti&&ti.value.trim()&&!state.titleBubbles.length){state.titleBubbles.push(ti.value.trim());ti.value='';renderTitleBubbles();liveUpdate();}const{str,url}=buildString();if(str){saveHist(str,url);const platformLabel=[...state.selPlatforms].map(k=>PLATFORMS[k]?.label||k).join(',');txrTrackEvent('search_open',{platforms:platformLabel,query_length:str.length});txrLogSearchEvent('open_google',{query:str,resultUrl:url,platforms:platformLabel});}return true;}
function copyStr(){const ti=document.getElementById('titleInput');if(ti&&ti.value.trim()&&!state.titleBubbles.length){state.titleBubbles.push(ti.value.trim());ti.value='';renderTitleBubbles();liveUpdate();}const{str}=buildString();if(!str){toast('Add a job title or skill first','err');return;}const lbl=document.getElementById('copyLbl');const showCopied=()=>{lbl.textContent='✓ Copied!';setTimeout(()=>lbl.textContent='Copy',2000);txrTrackTool('query_copied',{query_length:str.length},{query:str});};if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(str).then(()=>{showCopied();toast('Copied ✓','ok');}).catch(()=>fallbackCopy(str,showCopied));return;}fallbackCopy(str,showCopied);}
...
export { togglePlatform, focusBubble, relaxMustSkills, removeLastTitle, clearLoc, fixRegionMismatch, handleOpenClick, copyStr, resetAll };
