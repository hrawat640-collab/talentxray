...
import { esc, escapeRegExp, highlight } from './util.js';
...
function renderSuggestions(type,query){
  ...
  if(!items.length&&query){drop.innerHTML=`<div class="sug-empty">No suggestions — press Enter to add "${esc(query)}"</div>`;drop.classList.add('open');return;}
  ...
  Object.entries(grouped).forEach(([group,skills])=>{if(query&&Object.keys(grouped).length>1)html+=`<div class="sug-group-label">${group}</div>`;skills.forEach(({s,match})=>{const q=query.toLowerCase();const hi=q?s.replace(new RegExp(`(${escapeRegExp(q)})`,'gi'),'<em>$1</em>'):s;html+=`<div class="sug-item" onclick="pickSuggestion('${type}','${s.replace(/'/g,"\\'")}')"><div>${hi}</div><span>${group}</span></div>`;});});
